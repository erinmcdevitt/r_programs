...
---
---
title: "employment_and_productivity"
...
geometry: left = 0.5in, right = 0.5in, top =0.5in, bottom = 0.3in, asymmetric

  
```{r setup, include=FALSE, eval = FALSE, echo = FALSE, warning = FALSE, message = FALSE}
...
plot_jolts_unlevel <- jolts_unlevel %>% filter(date >= as.Date("2019-12-01")) %>%
  ...
  labs(title = 'Job Openings per Unemployed Person',
       subtitle = 'Ratio'
  ) +
  scale_x_date(breaks = my_breaks, labels = my_labels, expand = c(0,0)
  ) +
...
```{r p1, fig.width = 8.25, fig.height = 9.75}
# 
# mc1 <- list(plot_unrate, plot_earnings_unrate, plot_payroll, plot_lfpr)
# 
# employment <- plot_grid(mc1[[1]], mc1[[2]], mc1[[3]], mc1[[4]],
#                         align = 'hv', nrow = 2, ncol = 2)
# 
# # employment <- grid.arrange(plot_unrate, plot_earnings_unrate, plot_payroll, plot_lfpr,
# #                            nrow = 2, ncol = 2)
# 
# employment
